...
const computeHkdf = (key: any, salt: any, info: any, length: number) => {
...
export class ActiveCall extends EventEmitter {
	...
	engine: any
	_state: any
	_endResolver: ((reason: any) => void) | undefined
	_endPromise: Promise<any> | undefined
	_endTimer: any
	_ended: boolean
	_audioSource: string
	_audioStartDelayMs: number
	_playlist: any[]
	_feeder: any
	_idleTimer: any
	_endWhenQueueEmpty: boolean
	_idleGraceMs: number
	groupJid?: string

	end: () => void
	mute: (muted: any) => void
	waitForEnd: () => Promise<any> | undefined
	play: (source: any) => boolean
	enqueue: (source: any) => any
	skip: () => any
	queued: () => any
	nowPlaying: () => any
	_attachFeeder: (feeder: any) => void
	_clearIdleTimer: () => void
	_onTrackStart: (track: any) => void
	_onTrackEnd: (track: any) => void
	_onPlaylistIdle: () => void
	_updateState: (state: any) => void
	_emitAudio: (pcm: any) => void
	_forceEnd: (reason: any) => void

	constructor(callId: string, engine: any, durationMs: number) {
		super()
		this.callId = callId
		this.engine = engine
		this._state = CallState.Idle as any
		this._endResolver = undefined
		this._endPromise = undefined
		this._endTimer = null
		this._ended = false
		this._audioSource = 'silence'
		this._audioStartDelayMs = 0
		...
		this._attachFeeder = (feeder: any) => {
			this._feeder = feeder
		}

		this.play = (source: any) => {
			this._playlist = []
			if (this._feeder) {
		...
		this._onTrackStart = (track: any) => {
			this._clearIdleTimer()
			this.emit('track', track)
		}

		this._onTrackEnd = (track: any) => {
		...
		this._emitAudio = (pcm: any) => {
			this.emit('audio', pcm)
		}

		this._forceEnd = (reason: any) => {
			if (this._ended) return
			this._ended = true
			this._clearIdleTimer()
			if (this._endTimer) {
...
export class VoipClient {
	...
	_signaling: any
	_sock: any
	_activeCall: ActiveCall | null
	_externalSocket: boolean
	_capturePtr: number
	_captureChunkBytes: number
	_captureSampleRate: number
	_captureChannels: number
	_captureFramesPerChunk: number
	_feeder: any
	_wsCallHandler: ((node: any) => void) | null
	_wsReceiptHandler: ((node: any) => void) | null
	_captureStartRequested: boolean
	_audioStartTimer: any

	connect: () => Promise<void>
	attach: (socket: any) => Promise<void>
	call: (phoneNumber: any, opts?: any) => Promise<ActiveCall>
	callGroup: (groupJid: any, opts?: any) => Promise<ActiveCall>
	joinGroupCall: (invite: any, opts?: any) => Promise<ActiveCall>
	acceptCall: (isMicEnabled?: boolean) => any
	disconnect: () => void
	resetCallState: () => void
	_initVoipStack: () => Promise<void>
	_detachSocketHandlers: () => void
	_bindSocketHandlers: () => void
	_handleCallEvent: (eventType: any, eventData: any) => void
	_handleAudioCaptureInit: (config: any) => void
	_startAudioFeeder: () => void
	_handleAudioCaptureStart: () => void
	_handleAudioCaptureStop: () => void

	constructor(config: any = {}) {
		this._config = config
		this._log =
			typeof config.logger === 'function' ? config.logger : config.debug ? (...args) => console.log(...args) : () => {}
		this._engine = null
		this._relay = null
		...
		this.connect = async () => {
			if (!this._sock) {
				throw new Error(
					'VoipClient needs the socket you are already logged in with: new VoipClient({ socket: sock }). It never opens a session of its own, so there is no second pairing or QR scan.'
				)
			}
			if (!this._sock.ws) {
		...
		this.call = async (phoneNumber, opts = {}) => {
			...
			if (this._activeCall) {
				try {
					this._activeCall._forceEnd('replaced')
				} catch {}
				try {
					this._engine?.endCall?.(0, true)
				} catch {}
				try {
					this._feeder?.stop()
				} catch {}
			...
			if (!peerLid) {
				throw new Error(
					`Tidak bisa resolve LID untuk ${targetPnJid}. Pastikan target valid dan sudah tersimpan mapping LID/PN atau USync LID bisa diakses.`
				)
			}
			...
			for (const jid of [targetPnJid, peerLid]) {
				try {
					await this._sock.presenceSubscribe(jid)
				} catch {}
			}
			await new Promise(resolve => setTimeout(resolve, 150))

			let peerDeviceJids: any[] = []
			try {
			...
			let tcToken: any = null
			try {
				this._signaling.issueTcToken(peerLid).catch((err: any) => {
					this._log('[RTC ISSUE TC TOKEN ASYNC ERROR]', err?.message || err)
				})

				tcToken = await Promise.race([
					this._signaling.ensureTcToken(peerLid, targetPnJid).catch((err: any) => {
			...
			try {
				...
				if (this._activeCall === call) this._activeCall = null
				try {
					call._forceEnd('start_call_failed')
				} catch {}
				throw err
			}
		...
		this.callGroup = async (groupJid, opts = {}) => {
			...
			const members = (metadata?.participants ?? [])
				.map((participant: any) => toBareJid(participant.id ?? participant.jid ?? ''))
				.filter((jid: any) => jid && jid !== selfPn && jid !== selfLid)
			const wanted =
				Array.isArray(opts.participants) && opts.participants.length ? opts.participants.map(toBareJid) : members
			...
			try {
				...
				if (this._activeCall === call) this._activeCall = null
				try {
					call._forceEnd('start_group_call_failed')
				} catch {}
				throw err
			}
		...
		this.joinGroupCall = async (invite, opts = {}) => {
			...
			try {
				...
				if (this._activeCall === call) this._activeCall = null
				try {
					call._forceEnd('join_call_failed')
				} catch {}
				throw err
			}
		...
		this.disconnect = () => {
			...
			try {
				this._signaling?.attachEngine?.(null)
			} catch {}
			this._detachSocketHandlers()
		...
		this.resetCallState = () => {
			try {
				this._activeCall?._forceEnd('reset')
			} catch {}
			try {
				this._engine?.endCall?.(0, true)
			} catch {}
		...
		this._initVoipStack = async () => {
			...
			this._relay = new RelayRtcTransport({
				onTransportMessage: (data: any, ip: any, port: any) => this._engine?.handleOnTransportMessage(data, ip, port),
				onIceRtt: (rttMs: any, ip: any, port: any) => this._engine?.updateIceRtt(rttMs, ip, port)
			})

			this._engine = new WasmEngine({
				callbacks: {
					onSignalingXmpp: (peerJid: any, callId: any, xmlPayload: any) => {
					...
					onCallEvent: (eventType: any, eventData: any) => this._handleCallEvent(eventType, eventData),
					sendDataToRelay: (data: any, ip: any, port: any) => this._relay.send(data, ip, port),
					onAudioCaptureInit: (config: any) => this._handleAudioCaptureInit(config),
					onAudioCaptureStart: () => this._handleAudioCaptureStart(),
					onAudioCaptureStop: () => this._handleAudioCaptureStop(),
					onAudioPlaybackData: (audioData: any) => this._activeCall?._emitAudio(audioData),
			...
			const selfLidJid = this._sock.authState?.creds?.me?.lid || this._sock.authState?.creds?.me?.lidJid || ''
			...
			try {
				this._engine.updateNetworkMedium(2, 0)
			} catch {}
		...
		this._detachSocketHandlers = () => {
			...
			if (ws?.off) {
				try {
					if (this._wsCallHandler) ws.off('CB:call', this._wsCallHandler)
				} catch {}
				try {
					if (this._wsReceiptHandler) ws.off('CB:receipt', this._wsReceiptHandler)
				} catch {}
			}
			this._wsCallHandler = null
			this._wsReceiptHandler = null
		...
		this._handleAudioCaptureInit = (config: any) => {
		...
		this._startAudioFeeder = () => {
			...
			this._feeder = new AudioFeeder(
				...
				undefined,
				{
					onTrackStart: (track: any) => call?._onTrackStart(track),
					onTrackEnd: (track: any) => call?._onTrackEnd(track),
		...
		this._handleAudioCaptureStop = () => {
			...
			if (this._engine && this._capturePtr) {
				try {
					this._engine.free(this._capturePtr)
				} catch {}
				this._capturePtr = 0
			}
		}
...
export const makeVoipClient = async (socket: any, config: any = {}) => {
