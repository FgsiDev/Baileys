...
const setNodeChildren = (node: any, children: any[]) => {
	node.content = children.length ? children : undefined
}

const replaceNodeChild = (node: any, tag: string, nextChild: any) => {
...
const removeNodeChildrenByTag = (node: any, tag: string) => {
	setNodeChildren(
		node,
...
const parseCountAttr = (value: any, fallback = 0) => {
	const parsed = Number(value)
	return Number.isFinite(parsed) ? parsed : fallback
}

type TcTokenWaiter = (token: any) => void
...
export class SignalingBridge {
	...
	_baileys: any
	_voip: any
	_observedTcTokens: Map<string, { token: Buffer; timestamp: string }>
	_pendingTcTokenWaiters: Map<string, TcTokenWaiter[]>
	_ensuredSignalSessions: Map<string, number>
	_remoteDevicePeerByCallId: Map<string, string>
	_remoteObfuscatedPeerByCallId: Map<string, string>
	_remoteXmppRoutePeerByCallId: Map<string, string>
	_incomingCallPeerById: Map<string, string>
	_outgoingSignalingQueue: Promise<any>
	_incomingSignalingQueue: Promise<any>

	attachEngine: (voip: any) => void
	init: () => Promise<void>
	sendSignaling: (peerJid: any, callId: any, xmlPayload: any) => void
	processIncomingCall: (node: any, voip: any, activeCallId: any) => void
	processIncomingReceipt: (node: any, voip: any, activeCallId: any) => void
	requestTcToken: (jid: any) => Promise<any>
	ensureTcToken: (...jids: any[]) => Promise<any>
	discoverPeerDevices: (peerJid: any) => Promise<any[]>
	ensureSessionsForPeers: (jids: any[]) => Promise<void>
	resolveLid: (jid: any) => Promise<any>
	issueTcToken: (jid: any) => Promise<boolean>
	getRemoteDeviceJid: (callId: any) => string | undefined
	_doSendSignaling: (peerJid: any, callId: any, xmlPayload: any) => Promise<void>
	_sendCallStanza: (
		routeTo: any,
		voipNode: any,
		signalingTag: any,
		effectivePeerJid: any,
		callbackPeerJid: any
	) => Promise<void>
	_doProcessIncomingCall: (node: any, voip: any, activeCallId: any) => Promise<void>
	_doProcessIncomingReceipt: (node: any, voip: any, activeCallId: any) => Promise<void>
	_resolveLidViaUSync: (pnJid: any) => Promise<any>
	_normalizeLid: (value: any) => string | undefined
	_getStoredLidForPn: (pnJid: any) => Promise<any>
	_storeLidPnMapping: (lidJid: any, pnJid: any) => Promise<void>
	_maybeDecryptEnc: (voipNode: any, peerJid: any) => Promise<any>
	_encryptCallKey: (
		targetJid: any,
		rawCallKey: any,
		count: any
	) => Promise<{ encNode: any; shouldIncludeDeviceIdentity: boolean }>
	_ensureSignalSessions: (jids: any[], refresh?: boolean) => Promise<void>
	_appendDeviceIdentity: (voipNode: any) => void
	_toBareJid: (jid: any) => any
	_toCallDeviceJid: (jid: any) => any
	_toPrimaryDeviceJid: (jid: any) => any
	_hasConcreteDevice: (jid: any) => boolean
	_preferDeviceRouteJid: (...candidates: any[]) => string
	_preferOrderedRouteJid: (...candidates: any[]) => string
	_pickConcreteRouteHint: (...candidates: any[]) => string
	_resolveOutboundPeerJid: (callId: any, wasmPeerJid: any) => any
	_expandSignalSessionTargets: (jids: any[]) => any[]
	_normalizeStartCallPeerList: (jids: any[]) => string[]
	_findTcTokenNodes: (node: any) => any[]
	_processPrivacyTokenNotification: (node: any) => void
	_getTcTokenCandidateJids: (jid: any) => Promise<string[]>
	_storeTcToken: (jid: any, token: any, timestamp?: string) => Promise<Buffer>
	_waitForTcToken: (jid: any, timeoutMs?: number) => Promise<any>
	_rememberTcToken: (jid: any, token: any, timestamp?: string) => void
	_getTcToken: (jid: any) => Promise<any>
	...
	constructor(config: any) {
		...
		this.init = async () => {
			...
			if (this._sock.authState?.keys?.set) {
				...
				this._sock.authState.keys.set = async (data: any) => {
					const result = await originalKeysSet(data)

					for (const [jid, entry] of Object.entries<{ token?: any; timestamp?: any }>(data?.tctoken ?? {})) {
			...
			if (this._sock.ev?.on) {
				this._sock.ev.on('chats.update', (updates: any[]) => {
			...
			if (this._sock.ws?.on) {
				const handlePrivacyNotification = (node: any) => {
					try {
		...
		this.requestTcToken = async jid => {
			...
			const logResponse = (label: string, response: any) => {
				this._log(
					`[RTC TC TOKEN RESPONSE:${label}]`,
					JSON.stringify(
						response,
						(key, value) => {
			...
			const tryReadResponse = async (label: string, response: any) => {
			...
			for (const attempt of attempts) {
				...
				const waited = await Promise.race([waiter, new Promise(resolve => setTimeout(() => resolve(undefined), 2500))])
		...
		this.ensureTcToken = async (...jids) => {
			const uniqueJids = [...new Set(jids.map(jid => this._toBareJid(String(jid ?? '').trim())).filter(Boolean))]
		...
		this._resolveLidViaUSync = async pnJid => {
			...
			const query = new USyncQuery().withContext('message').withLIDProtocol()
			query.withUser(new USyncUser().withId(pnJid))

			try {
				const result = await this._sock.executeUSyncQuery(query)
				const row = (result?.list || []).find((item: any) => item?.id === pnJid || item?.lid)
		...
		this._storeLidPnMapping = async (lidJid, pnJid) => {
			...
			try {
				await this._sock.signalRepository?.lidMapping?.storeLIDPNMappings?.([{ lid: normalizedLid, pn: normalizedPn }])
			} catch {}
		...
		this._ensureSignalSessions = async (jids, refresh) => {
			...
			try {
				...
				for (const jid of missing) {
					try {
						this._ensuredSignalSessions.set(this._sock.signalRepository.jidToSignalProtocolAddress(jid), Date.now())
					} catch {}
				}
			} catch (err: any) {
		...
		this._normalizeStartCallPeerList = (jids: any[]): string[] => {
			const { jidDecode } = this._baileys
			...
			for (const jid of jids) {
				...
				if (decoded.device !== undefined && decoded.device !== null)
					result.add(`${decoded.user}:${decoded.device}@${server}`)
			}
			return [...result].slice(0, 5)
		}

		this._findTcTokenNodes = (node: any): any[] => {
			const result: any[] = []
			...
			for (const tokenNode of tokenNodes) {
				...
				if (jid && content instanceof Uint8Array && content.length > 0) {
					...
					this._storeTcToken(
						jid,
						Buffer.from(content),
						String(tokenNode.attrs?.t ?? Math.floor(Date.now() / 1000))
					).catch(() => {})
				}
			}
		}

		this._getTcTokenCandidateJids = async (jid: any): Promise<string[]> => {
			const result = new Set<string>()
			const bare = this._toBareJid(jid)

			if (bare) result.add(String(bare))
			...
			return new Promise(resolve => {
				const timer = setTimeout(() => {
					for (const candidate of candidates) {
						...
						if (!waiters) continue
						this._pendingTcTokenWaiters.set(
							candidate,
