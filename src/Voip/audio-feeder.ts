...
export class AudioFeeder {
	...
	rebuffers: number
	tracksPlayed: number
	_proc: ChildProcessWithoutNullStreams | null
	_playlist: any[]
	_current: any
	_sourceEnded: boolean
	_idle: boolean
	_pendingParts: Buffer[]
	...
	constructor(
		...
		onChunk: (chunk: Float32Array) => void,
		source: any = 'silence',
		hooks: { onTrackStart?: (track: any) => void; onTrackEnd?: (track: any) => void; onIdle?: () => void } = {}
	) {
	...
	_spawn(source: any) {
		...
		this._proc = proc
		proc.stdout.on('data', (chunk: Buffer) => this._absorb(chunk))
	...
	_resolveInputArgs(source: any = this.source) {
	...
	_takeChunk(): Float32Array {
		const frame =
			this._pendingParts.length === 1 && this._pendingParts[0]!.length === this._chunkBytes
				? (this._pendingParts.shift() as Buffer)
				: this._takeFromParts()
