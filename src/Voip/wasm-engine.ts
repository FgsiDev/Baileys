...
const parseJsonObjectEnv = (raw: any): any => {
	if (!raw) return {}
	try {
...
const filterWorkerStderr = (chunk: Buffer) => {
...
class NodeWorkerMessagePort {
	...
	constructor(worker: any, name = 'WAWebVoipWebWasmWorker') {
		this.#worker = worker
		this.name = name
		this.fullyConnected = new Promise<any>(resolve => {
	...
	addMessageListener = (type: any, handler: any) => {
	...
	removeMessageListener = (type: any, handler: any) => this.#listeners.get(type)?.delete(handler) ?? false
	removeAllMessageListeners = (type?: any) => {
...
export class WasmEngine {
	static #globalCallbackListeners: Map<string, Set<any>> = new Map()
	static #globalCallbacksRegistered = false
	static registerGlobalCallbackListener = (callbackName: any, handler: any) => {
	...
	static notifyGlobalCallbackListeners = (callbackName: any, data: any) => {
	...
	#instance: any = null
	#initialized = false
	#moduleRegistry: Map<string, any> = new Map()
	#vmContext: any = null
	#unusedWorkers: any[] = []
	#runningWorkers: any[] = []
	...
	#wasmModule: any = null
	#wasmMemory: any = null
	#removeRunDependencyCallback: any = null
	#workersLoadedCount = 0
	#audioPlaybackLoopInterval: any = null
	#audioPlaybackBuffer: any = null
	...
	constructor(config: any = {}) {
		...
		this.#config = {
			...
			storageDir: config.storageDir
				? path.isAbsolute(config.storageDir)
					? config.storageDir
					: path.resolve(process.cwd(), config.storageDir)
				: path.join(tmpdir(), 'elaina-voip'),
			wasmPath,
	...
	initialize = async (): Promise<void> => {
		...
		try {
			if (!fs.existsSync(voipStorageDir)) fs.mkdirSync(voipStorageDir, { recursive: true })
		} catch {}
		...
		if (!this.#config.wasmBinary && !fs.existsSync(this.#config.wasmPath)) {
			throw new Error(
				`WhatsApp VoIP WASM not found at ${this.#config.wasmPath}. Point wasmPath at it, hand the bytes over as wasmBinary, or put whatsapp.wasm, loader.js and worker-modules.js under <resourcesPath>/assets/wasm/.`
			)
		}
	...
	initVoipStack = (selfJid: any, meUserJid: any, selfLid: any) => {
	...
	joinOngoingCall = (options: any) => {
		...
		try {
			return this.#instance.joinVoipOngoingCall(
				...
				options.initialPeerJid ?? options.callCreatorJid,
				pnUserJids,
				lidUserJids,
				deviceJidsCsv,
				options.isVideo ?? false,
				options.groupJid ?? '',
				options.initialGroupTransactionId ?? 0,
				options.callCreatorIsNotContact ?? false,
				options.callLinkCode ?? '',
				options.isLightweight ?? false,
				options.scheduledId ?? '',
				options.joinAndAccept ?? true,
				options.chatName ?? '',
				options.lobbyEntryType ?? 0,
				options.isDeviceSwitch ?? false
			)
		} finally {
			pnUserJids?.delete?.()
			lidUserJids?.delete?.()
	...
	updateNetworkMedium = (networkMedium: any, networkMtu = 0) => {
		this.#ensureInitialized()
		this.#instance.updateNetworkMedium?.(networkMedium, networkMtu)
	}
	handleSignalingOffer = (msg: any) => {
		this.#ensureInitialized()
	...
	handleSignalingReceipt = (msg: any) => {
		this.#ensureInitialized()
		const tcTokenList = this.#createUint8List(msg.tcToken)
		try {
	...
	handleOnTransportMessage = (data: any, ip: any, port: any) => {
		this.#ensureInitialized()
	...
	updateIceRtt = (rttMs: any, relayIp: any, relayPort: any) => {
		this.#ensureInitialized()
		this.#instance.updateIceRtt?.(rttMs, relayIp, relayPort)
	}
	sendAudioData = (data: any, ptr: any) => {
		this.#ensureInitialized()
	...
	#registerGlobalCallbacks = () => {
		...
		const callbacksAny: any = callbacks
		WasmEngine.registerGlobalCallbackListener('loggingCallback', (data: any) => {
		...
		if (callbacks.onAudioCaptureInit) {
			WasmEngine.registerGlobalCallbackListener('initCaptureDriverJS', (data: any) => {
		...
		if (callbacks.onAudioPlaybackInit) {
			WasmEngine.registerGlobalCallbackListener('initPlaybackDriverJS', (data: any) => {
		...
		if (callbacks.onSignalingXmpp) {
			WasmEngine.registerGlobalCallbackListener('onSignalingXmpp', (data: any) => {
		...
		if (callbacks.onCallEvent) {
			WasmEngine.registerGlobalCallbackListener('onCallEvent', (data: any) => {
				callbacks.onCallEvent(data.eventType, data.eventDataJson)
			})
		}
		if (callbacks.sendDataToRelay) {
			WasmEngine.registerGlobalCallbackListener('sendDataToRelay', (data: any) => {
	...
	#requireModule = (name: any): any => {
		const preDefinedModules: Record<string, any> = {
			...
			bx: Object.assign((id: any) => String(id), { getURL: () => '' }),
		...
		const importDefaultFn = (dep: any): any => {
			const v = requireFn(dep)
			return v && v.__esModule ? v.default : v
		}
		const importAllFn = (dep: any): any => {
			const v = requireFn(dep)
	...
	#createVMContext = (memory: any) => {
		...
		const __d = (name: any, deps: any, factory: any) => {
			this.#moduleRegistry.set(name, { deps, factory, exports: undefined })
		}
		const babelHelpers = {
			extends: Object.assign,
			inheritsLoose: (sub: any, sup: any) => {
				sub.prototype = Object.create(sup.prototype)
				sub.prototype.constructor = sub
				sub.__proto__ = sup
			},
			objectWithoutPropertiesLoose: (source: any, excluded: any) => {
			...
			taggedTemplateLiteralLoose: (strings: any, raw: any) => {
	...
	#createAtomicsWrapper = (_memory: any) => {
		const atomicsWrapper: any = {
			...
			notify: (typedArray: any, index: any, count: any) => {
				try {
			...
			wait: (typedArray: any, index: any, value: any, timeout: any) => {
