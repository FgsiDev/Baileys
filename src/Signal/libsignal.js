...
export function makeLibSignalRepository(auth, onWhatsAppFunc) {
  ...
  const repository = {
    ...
    async encryptMessage({ jid, data }) {
      ...
      if (jid.includes("@s.whatsapp.net")) {
        ...
        if (lidForPN?.includes("@lid")) {
          ...
          const { [lidAddr.toString()]: lidSession } = await auth.keys.get(
            "session",
            [lidAddr.toString()],
          );
          if (lidSession) {
            ...
            const { [pnAddr.toString()]: pnSession } = await auth.keys.get(
              "session",
              [pnAddr.toString()],
            );
            if (pnSession) {
    ...
    async migrateSession(fromJid, toJid) {
      ...
      const { [lidAddr.toString()]: lidExists } = await auth.keys.get(
        "session",
        [lidAddr.toString()],
      );
      if (lidExists) {
      ...
      return parsedKeys.transaction(async () => {
        ...
        if (fromSession?.haveOpenSession()) {
          ...
          const copiedSession =
            libsignal.SessionRecord.deserialize(sessionBytes);
    ...
    async encryptMessageWithWire({ encryptionJid, wireJid, data }) {
      const result = await repository.encryptMessage({
        jid: encryptionJid,
        data,
      });
