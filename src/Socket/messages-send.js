...
export const makeMessagesSocket = (config) => {
  ...
  const assertSessions = async (jids, force) => {
    ...
    if (force) {
      ...
      const checkJidSession = async (jid) => {
        ...
        if (!hasSession) {
          ...
          if (hasSession) {
            logger.debug(
              { jid },
              "Found migrated LID session during force assert, skipping PN fetch",
            );
          }
        }
      ...
      const migrateDeviceToLid = async (jid, lidForPN) => {
        ...
        try {
          ...
          logger.debug(
            { fromJid: jid, toJid: lidWithDevice },
            "Migrated device session to LID",
          );
          // Delete PN session after successful migration
          try {
            await signalRepository.deleteSession(jid);
            logger.debug(
              { deletedPNSession: jid },
              "Deleted PN session after migration",
            );
          } catch (deleteError) {
            logger.warn(
              { jid, error: deleteError },
              "Failed to delete PN session",
            );
          }
        } catch (migrationError) {
          logger.warn(
            { jid, error: migrationError },
            "Failed to migrate device session",
          );
        }
      };
      ...
      for (const [user, userJids] of userGroups) {
        ...
        const checkMigratedSession = async (jid) => {
          ...
          if (
            ...
            const lidSignalId =
              signalRepository.jidToSignalProtocolAddress(lidWithDevice);
            ...
            if (hasSession) {
              logger.debug(
                { originalJid: jid, lidJid: lidWithDevice },
                "✅ Found bulk-migrated LID session",
              );
            }
          }
          // Add to fetch list if no session exists
    ...
    if (jidsRequiringFetch.length) {
      ...
      // Find overlaps
      const overlapping = Array.from(pnUsersBeingFetched).filter((user) =>
        lidUsersBeingFetched.has(user),
      );
  ...
  const createParticipantNodes = async (
    ...
    for (const patchedMessageWithJid of patched) {
      const { recipientJid: wireJid, ...patchedMessage } =
    ...
    const userEncryptionPromises = Array.from(devicesByUser.entries()).map(
      ([user, userDevices]) =>
        encryptionMutex.mutex(user, async () => {
          logger.debug(
            { user, deviceCount: userDevices.length },
          ...
          const getEncryptionJid = async (wireJid) => {
            ...
            try {
              ...
              try {
                ...
                logger.info(
                  { wireJid, lidWithDevice, isOwnDevice },
          ...
          for (const { recipientJid: wireJid, patchedMessage } of userDevices) {
            ...
            if (dsmMessage) {
              ...
              const isExactSenderDevice =
                wireJid === meId ||
                (authState.creds.me?.lid && wireJid === authState.creds.me.lid);
          ...
          logger.debug(
            { user, nodesCreated: userNodes.length },
  ...
  const relayMessageB = async (
    ...
    await authState.keys.transaction(async () => {
      ...
      if (isGroup || isStatus) {
        ...
        const groupAddressingMode =
          groupData?.addressingMode || (isLid ? "lid" : "pn");
            ...
            logger.debug(
              {
                ...
                devices: devices.map(
                  (d) =>
                    `${d.user}:${d.device}@${jidDecode(d.wireJid)?.server}`,
                ),
              },
        ...
        for (const { user, wireJid } of devices) {
          const isExactSenderDevice =
            wireJid === meId || (meLid && wireJid === meLid);
