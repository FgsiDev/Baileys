...
export const makeChatsSocket = (config) => {
  ...
  const fetchLidByJids = async (...jids) => {
    const validJids = jids.filter((jid) => isJidUser(jid));
    ...
    const result = await sock.executeUSyncQuery({
      ...
      parseUSyncQueryResult: (res) => {
        const usync = res?.content?.find((c) => c.tag === "usync");
        const list = usync?.content?.find((c) => c.tag === "list");
        const users = list?.content?.filter((c) => c.tag === "user") || [];
        return users.map((user) => ({
          jid: user?.attrs?.jid,
          lid: (user?.content || [])
            .filter((child) => child.tag === "lid")
            .map((lid) => lid?.attrs?.val)[0],
        }));
      },
    });
    return result || [];
