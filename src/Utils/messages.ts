...
export const downloadMediaMessage = async <Type extends 'buffer' | 'stream'>(
	...
	async function downloadMsg() {
		...
		const hasContextKey = 'contextInfo' in media && (media as any).contextInfo?.mediaDomainInfo?.e2EeMediaKey
		if (hasContextKey) {
			const origKey = 'mediaKey' in media ? (media as any).mediaKey : undefined
			const isEmpty = !origKey || (origKey instanceof Uint8Array && origKey.every((b: number) => b === 0))

			if (isEmpty) {
				download = {
					...(media as any),
					mediaKey: (media as any).contextInfo!.mediaDomainInfo!.e2EeMediaKey
				}
			}
		}
...
async function prepareStickerPackMessage(
	...
	try {
		...
		const thumbUpload = await encryptedStream(thumbnailBuffer, 'thumbnail-sticker-pack', {
			logger: options.logger,
			opts: options.options,
			mediaKey: Buffer.from(stickerPackUpload.mediaKey)
		})
