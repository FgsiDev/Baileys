  ...
  return {
      ...
      if (isInTransaction()) {
        ...
        if (idsRequiringFetch.length) {
          ...
          if (type === "sender-key") {
            logger.info(
              { idsRequiringFetch },
              "processing sender keys in transaction",
            );
            ...
            for (const senderKeyName of idsRequiringFetch) {
              await queueSenderKeyOperation(senderKeyName, async () => {
                logger.info(
                  { senderKeyName },
                  "fetching sender key in transaction",
                );
                ...
                logger.info(
                  { senderKeyName, hasResult: !!result[senderKeyName] },
                  "sender key fetch complete",
                );
              });
            }
          } else {
        ...
        if (type === "sender-key") {
          ...
          return await getKeyTypeMutex(type).runExclusive(() =>
            state.get(type, ids),
          );
        }
      }
    ...
    set: async (data) => {
      if (isInTransaction()) {
        ...
        const senderKeyNames = hasSenderKeys
          ? Object.keys(data["sender-key"] || {})
          : [];
        if (hasSenderKeys) {
          logger.info(
            { senderKeyNames },
            "processing sender key set operations",
          );
