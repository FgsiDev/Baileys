...
type EncryptedStreamOptions = {
	...
	logger?: ILogger
	opts?: RequestInit
	/** reuse an existing media key (eg. sticker pack thumbnail sharing the pack key) */
	mediaKey?: Buffer
}
...
export const downloadContentFromMessage = async (
	...
	// e2EeMediaKey from contextInfo overrides an empty/missing top-level mediaKey
	let resolvedMediaKey = mediaKey
	const isEmpty = !resolvedMediaKey || (resolvedMediaKey instanceof Uint8Array && resolvedMediaKey.every(b => b === 0))
	if (isEmpty && contextInfo?.mediaDomainInfo?.e2EeMediaKey) {
		resolvedMediaKey = contextInfo.mediaDomainInfo.e2EeMediaKey
	}

	const keys = await getMediaKeys(resolvedMediaKey, type)
