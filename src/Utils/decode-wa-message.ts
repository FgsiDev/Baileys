...
import {
	...
	decodeRichResponseMessage,
	decryptMsmsgBotMessage,
	type MsmsgMessageKey
} from './meta-ai-msmsg'
...
export function decodeMessageNode(stanza: BinaryNode, meId: string, meLid: string) {
	...
	if (isPnUser(from) || isLidUser(from) || isHostedLidUser(from) || isHostedPnUser(from)) {
		...
		fromMe = isJidNewsletter(from)
			? !!stanza.attrs?.is_sender
			: isLidUser(from)
				? areJidsSameUser(from, meLid)
				: areJidsSameUser(from, meId)
	} else {
					...
					try {
						...
						switch (e2eType) {
							...
							case 'msmsg': {
								...
								const secretIdCandidates = [botEditTargetId, metaTargetId, fullMessage.key?.id].filter(
									Boolean
								) as string[]
						...
						/*{
							const rich = (fullMessage.message as any)?.richResponseMessage
							if (rich && !rich.text) {
								const decoded = decodeRichResponseMessage(rich)
								if (decoded) rich.text = decoded
							}
							const editedRich = (fullMessage.message as any)?.protocolMessage?.editedMessage?.richResponseMessage
						...
						if (secret) {
							const secretBuf = Buffer.isBuffer(secret)
								? secret
								: Buffer.from(
										(secret as Uint8Array).buffer,
										(secret as Uint8Array).byteOffset,
										(secret as Uint8Array).byteLength
									)
